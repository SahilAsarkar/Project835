...
export default function FilesView({ clients = [], activeClientId, onSelectClient }) {
  ...
  async function handleFileUpload(e) {
    ...
    try {
      ...
      setSuccessMessage(`File '${file.name}' uploaded and registered successfully.`);
    ...
    <section className="view on" id="v-files">
      <div className="hdr-row">
        <div>
          ...
          <p className="sub">Uploaded files, data exports, and associated resources for <b>{currentClient?.name}</b>.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="stub" style={{ textAlign: 'center', padding: '36px' }}>
          ...
          <p style={{ margin: '6px 0 0', color: 'var(--ink-2)' }}>
            Upload files or resources for {currentClient?.name}.
          </p>
        </div>
      ) : (
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {documents.map((doc) => {
              const ext = (doc.original_filename?.split('.').pop() || 'FILE').toUpperCase();
              return (
                <tr key={doc.id}>
                  <td>
